...
export const joinPartnerWithCode = async (userId: string, inviteCode: string) => {
  ...
  if (!data.success) return { data: null, error: new Error(data.error) };

  // Fetch the updated joint account to return
  const { data: jointAccount, error: jaError } = await supabase
    .from('joint_accounts')
    .select('*')
    .eq('id', data.joint_account_id)
    .single();

  return { data: jointAccount, error: jaError };
};
